...
> **A decode-boundary refusal is a REFUSAL, not silence `[MUST]` (0.8.2.24).** §4.9(c)'s deliver-or-signal rule is scoped to *"every request the peer admits"*, and a frame refused at decode was never admitted — so this seam is stated here rather than inherited from it. A peer that refuses at the decode boundary **MUST** emit the coded response above, correlated by `request_id` where the id is available, and otherwise **MUST** make a best-effort coded frame before closing, per §4.10(a)'s emission shape for the same situation. **Dropping the frame with no response and no close is non-conformant, and so is closing with no coded frame** — a bare socket close is indistinguishable from a network fault (§4.6), and on a **multiplexed** connection it additionally discards unrelated *admitted* requests, which §4.9(c) independently forbids for each of them. **Whether the peer closes afterwards remains its own choice**; the landed `[MUST]` above that a uniform verdict MUST NOT be required is untouched. **A map-wide precondition inside `verify_request` is conformant only if it preserves these classes** — it is the one placement that can manufacture a verdict this enumeration does not assign, so such a peer MUST still answer auth-class for an author forgery and authz-class for a capability or chain forgery. The **grantee** arm is the existing `401 unresolvable_grantee` carve-out, unchanged.
