...
| **`signature`** | Deterministic Ed25519 sign/verify over an entity's **`content_hash`**, per `ENTITY-CORE-PROTOCOL.md` §7.3 — the signed message is `varint(format_code) ‖ SHA256(ECF({type, data}))` — **the same value the `content_hash` category above produces**, whose length follows its own leading format byte and is never a constant (§8.4.5) — and **not** the ECF bytes themselves. Vectors use fixed deterministic Ed25519 seeds (named in `.diag` as hex literals); Ed25519's RFC 8032 signing is deterministic by construction, so produced signatures are reproducible across impls. Each vector encodes the signed entity per Class A, derives its `content_hash` per §7.1, signs that value, and verifies; the `canonical` output bytes are the produced signature. **At least one vector MUST sign an entity that also appears in the `content_hash` category**, so the two categories cross-check by inspection: that vector's signed message *is* the other's `canonical` output. *(0.8.2.26 — this category read "signs the canonical bytes", which is a different message from the one `ENTITY-CORE-PROTOCOL.md` §7.3 specifies, and the fixture was generated to match it. Class B composes the rules stated elsewhere and does not define them — see the composition rule below — so this was a consumer of §7.3 implementing something else. The three `signature.*` vectors are corrected in place per the corpus changelog's standing rule; the cross-check vector is added because nothing related the two categories and the divergence sat inside one artifact, unreadable from either side.)* | `encode_equal` |
