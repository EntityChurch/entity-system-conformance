#!/bin/sh
# py-prototype launcher — the one file an instrument slot execs. The bundle sits beside it at "$0.d".
#
...
B="$0.d"
[ -d "$B/pyrt" ] || { echo "py-prototype: bundle not found at $B (run make build)" >&2; exit 3; }
