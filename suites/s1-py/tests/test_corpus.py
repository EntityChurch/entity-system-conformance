...
from s1py import cbor, ed25519, ident  # noqa: E402

ROOT = Path(__file__).resolve().parents[3]
SNAP = ROOT / "spec-data" / "core-0.8.2.21"
CORPUS = SNAP / "test-vectors" / "ecf-conformance" / "conformance-vectors.cbor"
...
class Corpus(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.raw, cls.vectors, cls.findings = _load()
    ...
    def test_artifact_is_the_pinned_one(self):
        ...
        self.assertEqual(self.findings.tags, [])
    ...
    def test_every_vector_accounted_for(self):
        ...
        print(f"\necf corpus: {len(ids)} vectors — {len(ran)} run, {len(NOT_RUN)} declared not-run", file=sys.stderr)
